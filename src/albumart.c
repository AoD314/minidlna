...
static int art_cache_exists(const char* orig_path, char** cache_file) {
...
void update_if_album_art(const char* path) {
    ...
    while ((dp = readdir(dh)) != NULL) {
        ...
        if (((is_video(dp->d_name) && (dir_type & TYPE_VIDEO)) || (is_audio(dp->d_name) && (dir_type & TYPE_AUDIO))) && (album_art || strncmp(dp->d_name, match, ncmp) == 0)) {
            snprintf(file, sizeof(file), "%s/%s", dir, dp->d_name);
...
char* check_embedded_art(const char* path, uint8_t* image_data, int image_size) {
...
static char* check_for_album_file(const char* path) {
    char file[MAXPATHLEN];
...
int64_t find_album_art(const char* path, uint8_t* image_data, int image_size) {
    ...
    if ((image_size && (album_art = check_embedded_art(path, image_data, image_size))) || (album_art = check_for_album_file(path))) {
