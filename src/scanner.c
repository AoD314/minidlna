...
int insert_container(const char* item,
...
static void insert_containers(const char* name, const char* path, const char* refID, const char* class, int64_t detailID) {
    char sql[128];
    char** result;
    int ret;
    int cols, row;
        ...
        if (artist) {
            ...
            if (valid_cache && strcmp(album ? album : _("Unknown Album"), last_artistAlbum.name) == 0) {
                ...
                insert_container(album ? album : _("Unknown Album"),
                                 ...
                                 album ? last_album.parentID : NULL,
                                 "album.musicAlbum",
                                 artist,
                                 genre,
                                 album_art,
...
int64_t insert_directory(const char* name, const char* path, const char* base, const char* parentID, int objectID) {
    int64_t detailID = 0;
...
int insert_file(const char* name, const char* path, const char* parentID, int object, media_types types) {
    const char* class;
    char objectID[64];
    int64_t detailID = 0;
    char base[8];
...
static int filter_type(scan_filter* d) {
#if HAVE_STRUCT_DIRENT_D_TYPE
    return ((d->d_type == DT_DIR) || (d->d_type == DT_LNK) || (d->d_type == DT_UNKNOWN));
#else
    return 1;
#endif
}
...
static int filter_a(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && (is_audio(d->d_name) || is_playlist(d->d_name)))));
}

static int filter_av(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && (is_audio(d->d_name) || is_video(d->d_name) || is_playlist(d->d_name)))));
}

static int filter_ap(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && (is_audio(d->d_name) || is_image(d->d_name) || is_playlist(d->d_name)))));
}

static int filter_v(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && is_video(d->d_name))));
}

static int filter_vp(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && (is_video(d->d_name) || is_image(d->d_name)))));
}

static int filter_p(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && is_image(d->d_name))));
}

static int filter_avp(scan_filter* d) {
    return (filter_hidden(d) && (filter_type(d) || (is_reg(d) && (is_audio(d->d_name) || is_image(d->d_name) || is_video(d->d_name) || is_playlist(d->d_name)))));
}

static void ScanDirectory(const char* dir, const char* parent, media_types dir_types) {
...
static int cb_orphans(void* args, int argc, char** argv, char** azColName) {
