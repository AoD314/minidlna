...
static void my_dst_mgr_init(j_compress_ptr cinfo) {
...
static boolean my_dst_mgr_empty(j_compress_ptr cinfo) {
    struct my_dst_mgr* dst = (void*)cinfo->dest;
...
static void my_dst_mgr_term(j_compress_ptr cinfo) {
    struct my_dst_mgr* dst = (void*)cinfo->dest;
...
void image_free(image_s* pimage) {
    free(pimage->buf);
    free(pimage);
}

pix get_pix(image_s* pimage, int32_t x, int32_t y) {
    if (x < 0) {
        x = 0;
...
void put_pix_alpha_replace(image_s* pimage, int32_t x, int32_t y, pix col) {
...
image_s* image_new_from_jpeg(const char* path, int is_file, const uint8_t* buf, int size, int scale, int rotate) {
    image_s* vimage;
    FILE* file = NULL;
...
void image_upsize(image_s* pdest, image_s* psrc, int32_t width, int32_t height) {
    int32_t vx, vy;
    ...
    for (vy = 0; vy < height; vy++) {
        for (vx = 0; vx < width; vx++) {
            ...
            vcol = COL_FULL((uint8_t)((COL_RED(vcol1) * (1.0 - x_dist) + COL_RED(vcol2) * (x_dist)) * (1.0 - y_dist)
                                      + (COL_RED(vcol3) * (1.0 - x_dist) + COL_RED(vcol4) * (x_dist)) * (y_dist)),
                            (uint8_t)((COL_GREEN(vcol1) * (1.0 - x_dist) + COL_GREEN(vcol2) * (x_dist)) * (1.0 - y_dist)
                                      + (COL_GREEN(vcol3) * (1.0 - x_dist) + COL_GREEN(vcol4) * (x_dist)) * (y_dist)),
                            (uint8_t)((COL_BLUE(vcol1) * (1.0 - x_dist) + COL_BLUE(vcol2) * (x_dist)) * (1.0 - y_dist)
                                      + (COL_BLUE(vcol3) * (1.0 - x_dist) + COL_BLUE(vcol4) * (x_dist)) * (y_dist)),
                            (uint8_t)((COL_ALPHA(vcol1) * (1.0 - x_dist) + COL_ALPHA(vcol2) * (x_dist)) * (1.0 - y_dist)
                                      + (COL_ALPHA(vcol3) * (1.0 - x_dist) + COL_ALPHA(vcol4) * (x_dist)) * (y_dist)));
#endif
...
void image_downsize(image_s* pdest, image_s* psrc, int32_t width, int32_t height) {
    int32_t vx, vy;
    pix vcol;
    int32_t i, j;
#if !defined __i386__ && !defined __x86_64__
        ...
        for (vx = 0; vx < width; vx++) {
            ...
            for (j = 0; j < half_square_height << 1; j++) {
                for (i = 0; i < half_square_width << 1; i++) {
                    vcol = get_pix(psrc, ((int32_t)rx) - half_square_width + i, ((int32_t)ry) - half_square_height + j);

                    if (((j == 0) || (j == (half_square_height << 1) - 1)) && ((i == 0) || (i == (half_square_width << 1) - 1))) {
...
#endif
            put_pix_alpha_replace(pdest, vx, vy, COL_FULL((uint8_t)red, (uint8_t)green, (uint8_t)blue, (uint8_t)alpha));
        }
    }
}
