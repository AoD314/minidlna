...
int dlna_timestamp_is_present(const char* filename, int* raw_packet_size) {
    ...
    for (i = 0; i < MPEG_TS_PACKET_LENGTH_DLNA; i++) {
        if (buffer[i] == MPEG_TS_SYNC_CODE) {
            if (buffer[i + MPEG_TS_PACKET_LENGTH_DLNA] == MPEG_TS_SYNC_CODE && buffer[i + MPEG_TS_PACKET_LENGTH_DLNA * 2] == MPEG_TS_SYNC_CODE) {
                *raw_packet_size = MPEG_TS_PACKET_LENGTH_DLNA;
                if (buffer[i + MPEG_TS_PACKET_LENGTH] == 0x00 && buffer[i + MPEG_TS_PACKET_LENGTH + 1] == 0x00 && buffer[i + MPEG_TS_PACKET_LENGTH + 2] == 0x00
                    && buffer[i + MPEG_TS_PACKET_LENGTH + 3] == 0x00) {
                    return 0;
                } else {
                    return 1;
                }
            } else if (buffer[i + MPEG_TS_PACKET_LENGTH] == MPEG_TS_SYNC_CODE && buffer[i + MPEG_TS_PACKET_LENGTH * 2] == MPEG_TS_SYNC_CODE) {
...
void check_for_captions(const char* path, int64_t detailID) {
    ...
    if (!detailID) {
        detailID = sql_get_int64_field(db,
                                       "SELECT ID from DETAILS where (PATH > '%q.' and PATH <= '%q.z')"
                                       " and MIME glob 'video/*' limit 1",
                                       file,
                                       file);
        if (detailID <= 0) {
...
static void parse_nfo(const char* path, metadata_t* m) {
    FILE* nfo;
    char* buf;
    ...
    if (stat(path, &file) != 0 || file.st_size > 65535) {
...
void free_metadata(metadata_t* m, uint32_t flags) {
    if (flags & FLAG_TITLE) {
        free(m->title);
    }
...
int64_t GetFolderMetadata(const char* name, const char* path, const char* artist, const char* genre, int64_t album_art) {
    int ret;
...
int64_t GetAudioMetadata(const char* path, const char* name) {
    ...
    } else if (ends_with(path, ".m4a") || ends_with(path, ".mp4") || ends_with(path, ".aac") || ends_with(path, ".m4p")) {
        strcpy(type, "aac");
...
int64_t GetVideoMetadata(const char* path, const char* name) {
    ...
    for (i = 0; i < ctx->nb_streams; i++) {
        if (lav_codec_type(ctx->streams[i]) == AVMEDIA_TYPE_AUDIO && audio_stream == -1) {
            ...
            continue;
        } else if (lav_codec_type(ctx->streams[i]) == AVMEDIA_TYPE_VIDEO && !lav_is_thumbnail_stream(ctx->streams[i], &m.thumb_data, &m.thumb_size) && video_stream == -1) {
            video_stream = i;
    ...
    if (astream) {
        ...
        case AV_CODEC_ID_AAC:
            ...
            case AAC_LC_ER:
                ...
                /* AAC @ Level 1/2 */
                if (lav_channels(astream) <= 2 && lav_bit_rate(astream) <= 576000) {
        ...
        case AV_CODEC_ID_AMR_NB:
            ...
            break;
        default:
            if ((lav_codec_id(astream) >= AV_CODEC_ID_PCM_S16LE) && (lav_codec_id(astream) < AV_CODEC_ID_ADPCM_IMA_QT)) {
    ...
    if (vstream) {
        ...
        if (strcmp(ctx->iformat->name, "avi") == 0) {
            ...
            if (lav_codec_id(vstream) == AV_CODEC_ID_MPEG4) {
                ...
                if (memcmp(fourcc, "XVID", 4) == 0 || memcmp(fourcc, "DX50", 4) == 0 || memcmp(fourcc, "DIVX", 4) == 0) {
        ...
        case AV_CODEC_ID_MPEG1VIDEO:
            if (strcmp(ctx->iformat->name, "mpeg") == 0) {
                if ((lav_width(vstream) == 352) && (lav_height(vstream) <= 288)) {
        ...
        case AV_CODEC_ID_MPEG2VIDEO:
            ...
            if (strcmp(ctx->iformat->name, "mpegts") == 0) {
                ...
                if ((lav_width(vstream) >= 1280) && (lav_height(vstream) >= 720)) {
                    off += sprintf(m.dlna_pn + off, "HD_NA");
                } else {
                    off += sprintf(m.dlna_pn + off, "SD_");
                    if ((lav_height(vstream) == 576) || (lav_height(vstream) == 288)) {
        ...
        case AV_CODEC_ID_H264:
            ...
            if (strcmp(ctx->iformat->name, "mpegts") == 0) {
                ...
                if (((((lav_width(vstream) == 1920 || lav_width(vstream) == 1440) && lav_height(vstream) == 1080) || (lav_width(vstream) == 720 && lav_height(vstream) == 480))
                     && fps == 59 && interlaced)
                    || ((lav_width(vstream) == 1280 && lav_height(vstream) == 720) && fps == 59 && !interlaced)) {
                    if ((lav_profile(vstream) == FF_PROFILE_H264_MAIN || lav_profile(vstream) == FF_PROFILE_H264_HIGH) && audio_profile == PROFILE_AUDIO_AC3) {
                        off += sprintf(m.dlna_pn + off, "HD_60_");
                        lav_profile(vstream) = FF_PROFILE_SKIP;
                    }
                } else if (((lav_width(vstream) == 1920 && lav_height(vstream) == 1080) || (lav_width(vstream) == 1440 && lav_height(vstream) == 1080)
                            || (lav_width(vstream) == 1280 && lav_height(vstream) == 720) || (lav_width(vstream) == 720 && lav_height(vstream) == 576))
                           && interlaced && fps == 50) {
                    if ((lav_profile(vstream) == FF_PROFILE_H264_MAIN || lav_profile(vstream) == FF_PROFILE_H264_HIGH) && audio_profile == PROFILE_AUDIO_AC3) {
                ...
                case FF_PROFILE_H264_CONSTRAINED_BASELINE:
                    ...
                    if (lav_width(vstream) <= 352 && lav_height(vstream) <= 288 && lav_bit_rate(vstream) <= 384000) {
                        off += sprintf(m.dlna_pn + off, "CIF15_");
                        break;
                    } else if (lav_width(vstream) <= 352 && lav_height(vstream) <= 288 && lav_bit_rate(vstream) <= 3000000) {
                ...
                case FF_PROFILE_H264_MAIN:
                    ...
                    if (lav_profile(vstream) != FF_PROFILE_H264_BASELINE && lav_profile(vstream) != FF_PROFILE_H264_CONSTRAINED_BASELINE
                    ...
                    if (lav_width(vstream) <= 720 && lav_height(vstream) <= 576 && lav_bit_rate(vstream) <= 10000000) {
                        off += sprintf(m.dlna_pn + off, "SD_");
                    } else if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1152 && lav_bit_rate(vstream) <= 20000000) {
                        off += sprintf(m.dlna_pn + off, "HD_");
                    } else {
                        DPRINTF(E_DEBUG,
                                L_METADATA,
                                "Unsupported h.264 video profile! [%s, %dx%d, %lldbps : %s]\n",
                                m.dlna_pn,
                                lav_width(vstream),
                                lav_height(vstream),
                                (long long)lav_bit_rate(vstream),
                                basepath);
                        free(m.dlna_pn);
                        m.dlna_pn = NULL;
                    }
                ...
                case FF_PROFILE_H264_HIGH:
                    ...
                    if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1152 && lav_bit_rate(vstream) <= 30000000 && audio_profile == PROFILE_AUDIO_AC3) {
                        off += sprintf(m.dlna_pn + off, "HD_");
                    } else {
                ...
                if (raw_packet_size == MPEG_TS_PACKET_LENGTH_DLNA) {
                    if (lav_profile(vstream) == FF_PROFILE_H264_HIGH || dlna_ts_present) {
                        ts_timestamp = VALID;
                    } else {
                        ts_timestamp = EMPTY;
                    }
                ...
                case FF_PROFILE_H264_CONSTRAINED_BASELINE:
                    ...
                    } else if (lav_width(vstream) <= 720 && lav_height(vstream) <= 576) {
                        if (lav_level(vstream) == 30 && audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 5000000) {
                            off += sprintf(m.dlna_pn + off, "BL_L3L_SD_AAC");
                        } else if (lav_level(vstream) <= 31 && audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 15000000) {
                            off += sprintf(m.dlna_pn + off, "BL_L31_HD_AAC");
                        } else {
                            goto mp4_mp_fallback;
                        }
                    } else if (lav_width(vstream) <= 1280 && lav_height(vstream) <= 720) {
                        if (lav_level(vstream) <= 31 && audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 15000000) {
                            off += sprintf(m.dlna_pn + off, "BL_L31_HD_AAC");
                        } else if (lav_level(vstream) <= 32 && audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 21000000) {
                ...
                mp4_mp_fallback:
                    ...
                    if (lav_width(vstream) <= 720 && lav_height(vstream) <= 576 && lav_bit_rate(vstream) <= 10000000) {
                        ...
                        if (audio_profile == PROFILE_AUDIO_AC3) {
                            off += sprintf(m.dlna_pn + off, "AC3");
                        } else if (audio_profile == PROFILE_AUDIO_AAC || audio_profile == PROFILE_AUDIO_AAC_MULT5) {
                            off += sprintf(m.dlna_pn + off, "AAC_MULT5");
                    ...
                    } else if (lav_width(vstream) <= 1280 && lav_height(vstream) <= 720 && lav_bit_rate(vstream) <= 15000000 && audio_profile == PROFILE_AUDIO_AAC) {
                        off += sprintf(m.dlna_pn + off, "HD_720p_AAC");
                    } else if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1080 && lav_bit_rate(vstream) <= 21000000 && audio_profile == PROFILE_AUDIO_AAC) {
                    ...
                    break;
                case FF_PROFILE_H264_HIGH:
                    if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1080 && lav_bit_rate(vstream) <= 25000000 && audio_profile == PROFILE_AUDIO_AAC) {
        ...
        case AV_CODEC_ID_MPEG4:
            ...
            if (strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2") == 0) {
                ...
                if (ends_with(path, ".3gp")) {
                    ...
                    if (ctx->bit_rate <= 1000000 && audio_profile == PROFILE_AUDIO_AAC) {
                        off += sprintf(m.dlna_pn + off, "MP4_ASP_AAC");
                    } else if (ctx->bit_rate <= 4000000 && lav_width(vstream) <= 640 && lav_height(vstream) <= 480 && audio_profile == PROFILE_AUDIO_AAC) {
                        off += sprintf(m.dlna_pn + off, "MP4_SP_VGA_AAC");
                    } else {
                        DPRINTF(E_DEBUG, L_METADATA, "Unsupported h.264 video profile! [%dx%d, %lldbps]\n", lav_width(vstream), lav_height(vstream), (long long)ctx->bit_rate);
                        free(m.dlna_pn);
                        m.dlna_pn = NULL;
                    }
                }
        ...
        case AV_CODEC_ID_VC1:
            ...
            if ((lav_width(vstream) <= 176) && (lav_height(vstream) <= 144) && (lav_level(vstream) == 0)) {
            ...
            } else if ((lav_width(vstream) <= 352) && (lav_height(vstream) <= 288) && (lav_profile(vstream) == 0) && (ctx->bit_rate / 8 <= 384000)) {
            ...
            } else if ((lav_width(vstream) <= 720) && (lav_height(vstream) <= 576) && (ctx->bit_rate / 8 <= 10000000)) {
            ...
            } else if ((lav_width(vstream) <= 1920) && (lav_height(vstream) <= 1080) && (ctx->bit_rate / 8 <= 20000000)) {
